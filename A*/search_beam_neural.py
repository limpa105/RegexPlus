...
def search(examples: List[str], beam_size: int) -> State:
    ...
    if len(beams[sum_lengths]) == 0:
        raise Exception("No regex works! (This is the NN's fault)")
    return min(beams[sum_lengths], key=lambda s: s.score)


def next_states(examples: List[str], state: State) -> Iterator[Tuple[VSAState, NNState, Regex]]:
    for r, nn_state in state.nn_state.best_next_edges(NUM_NEXT_CHARS):
