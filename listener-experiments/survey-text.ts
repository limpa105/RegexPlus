...
const info = <const>{
  ...
  parameters: {
    ...
    userID: {
      type: ParameterType.STRING,
      pretty_name: "user_id",
      defaulr: "something went horribly wrong"
    },
    data_id: {
      type: ParameterType.INT,
...
class SurveyTextPlugin implements JsPsychPlugin<Info> {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    display_element.querySelector("#jspsych-survey-text-form").addEventListener("submit", (e) => {
      ...
      // next trial
      // TODO: send the question_data to the server, along with the parameters
      // that indicate the user id and example id.
      // TODO: take parameters that indicate the user id and example id
      this.saveData({
        user_id: trial.userID,
        regex: trial.regex,
        data_id: trial.data_id,
        example_index: trial.example_id,
        data: question_data,
      });
