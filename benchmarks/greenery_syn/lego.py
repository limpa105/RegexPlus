...
class conc(lego):
    ...
    def reduce(self):
        ...
        # If R's language is a subset of S's language, then R?S+ and S+R? are
        # both equivalent to just S+. And this generalizes!
        if len(self.mults) > 1:
            for i in range(len(self.mults) - 1):
                left = self.mults[i]
                right = self.mults[i+1]
                for p in ((left, right), (right, left)):
                    first_mul = p[0].multiplicand
                    second_mul = p[1].multiplicand
                    if first_mul.intersection(second_mul).equivalent(first_mul) \
                      and p[0].multiplier.min == bound(0) \
                      and p[0].multiplier.max <= p[1].multiplier.max:
                        # new max is the sum of the old maxes;
                        # p[0]'s multiplicand can be freely forgotten
                        squished = mult(
                            second_mul,
                            p[0].multiplier + p[1].multiplier
                        )
