...
"""
Our synthesizer totally can't be fully expressive.
But in practice, this is only a problem if the end-user can't get the regex they want.
It thus makes sense to potentially offer these common complex regexes pre-packed!
"""

# from https://github.com/madisonmay/CommonRegex/blob/master/commonregex.py
fulldate         = re.compile('(?:(?<!\:)(?<!\:\d)[0-3]?\d(?:st|nd|rd|th)?\s+(?:of\s+)?(?:jan\.?|january|feb\.?|february|mar\.?|march|apr\.?|april|may|jun\.?|june|jul\.?|july|aug\.?|august|sep\.?|september|oct\.?|october|nov\.?|november|dec\.?|december)|(?:jan\.?|january|feb\.?|february|mar\.?|march|apr\.?|april|may|jun\.?|june|jul\.?|july|aug\.?|august|sep\.?|september|oct\.?|october|nov\.?|november|dec\.?|december)\s+(?<!\:)(?<!\:\d)[0-3]?\d(?:st|nd|rd|th)?)(?:\,)?\s*(?:\d{4})?|[0-3]?\d[-\./][0-3]?\d[-\./]\d{2,4}', re.IGNORECASE)
...
# selections from https://projects.lukehaas.me/regexhub/
dmy_date         = re.compile(r'^(0?[1-9]|[12][0-9]|3[01])([ \/\-])(0?[1-9]|1[012])\2([0-9][0-9][0-9][0-9])(([ -])([0-1]?[0-9]|2[0-3]):[0-5]?[0-9]:[0-5]?[0-9])?$')
twentyfour_time  = re.compile(r'^([01]?[0-9]|2[0-3]):[0-5][0-9]$')
html             = re.compile(r'^<([a-z1-6]+)([^<]+)*(?:>(.*)<\/\1>| *\/>)$')
youtube          = re.compile(r'https?:\/\/(www\.)?youtube.com\/channel\/UC([-_a-z0-9]{22})', re.IGNORECASE)
css_comment      = re.compile(r'\/\*[^*]*\*+([^\/*][^*]*\*+)*\/')
img_shortcode    = re.compile(r'\[img\](.*?)\[\/img\]')
bf_code          = re.compile(r'^[+-<>.,\[\] \t\n\r]+$')
